...
MOTO_REAL_INFO MOTOR_REAL_INFO[11] = {0}; /* 0-2对应三个轮子，3云台电机，4、5臂旋转
											 6、7夹爪抬升，8-11发射电机*/

pid MOTOR_PID_RPM[11]={0};	//速度pid信息 1-4底盘电机 5-6发射电机
pid MOTOR_PID_POS[11]={0};	//位置pid信息
...
void m3508_Init()
{
	...
	MOTOR_REAL_INFO[4].unitMode=SPEED_CONTROL_MODE;
	MOTOR_REAL_INFO[5].unitMode=SPEED_CONTROL_MODE;
	/* 位置模式 */
	...
	/* pid参数初始化 */
//	/* 速度模式初始化 */
	PID_parameter_init(&MOTOR_PID_RPM[0],11.2f,1.1f,0,20000,20000,-0.5);
	PID_parameter_init(&MOTOR_PID_RPM[1],11.2f,1.1f,0,20000,20000,-0.5);
	
	PID_parameter_init(&MOTOR_PID_RPM[2],14.0f,1.2f,0,20000,20000,-0.5);
	PID_parameter_init(&MOTOR_PID_RPM[3],14.0f,1.2f,0,20000,20000,-0.5);

	PID_parameter_init(&MOTOR_PID_RPM[4] , 12.0f, 1.0f, 0.3f, 20000, 20000, 1);
	PID_parameter_init(&MOTOR_PID_RPM[5] , 12.0f, 1.0f, 0.3f, 20000, 20000, 1);
	...
	MOTOR_REAL_INFO[0].LAST_ANGLE=MOTOR_REAL_INFO[0].FIRST_ANGLE_INTEGRAL_FLAG=MOTOR_REAL_INFO[0].REAL_ANGLE=0;
	MOTOR_REAL_INFO[1].LAST_ANGLE=MOTOR_REAL_INFO[1].FIRST_ANGLE_INTEGRAL_FLAG=MOTOR_REAL_INFO[1].REAL_ANGLE=0;
}

/*!
 * \fn     m3508_update_m3508_info
 * \brief  更新CAN1上电机的报文并且根据id存到不同的包
           里
 *          
...
/*!
 * \fn     down_update_info
 * \brief  更新CAN2上电机的报文并且根据id存到不同的包
           里
 *          
 * \param  [in] CAN_RxHeaderTypeDef *msg   #
 * \param  [in] uint8_t	can1_RxData[8]     #
...
void down_update_info(CAN_RxHeaderTypeDef *msg,uint8_t	can2_RxData[8])
{
	switch(msg -> StdId)  // 检测标准ID
	{
    	case M3508_CHASSIS_MOTOR_ID_1:
		{ 
			MOTOR_REAL_INFO[6].ANGLE   = (can2_RxData[0] << 8) | can2_RxData[1];  // 转子机械角度
			MOTOR_REAL_INFO[6].RPM     = (can2_RxData[2] << 8) | can2_RxData[3];  // 实际转子转速
			MOTOR_REAL_INFO[6].CURRENT = (can2_RxData[4] << 8) | can2_RxData[5];  // 实际转矩电流
		}; break;
		
		case M3508_CHASSIS_MOTOR_ID_2:
		{ 
			MOTOR_REAL_INFO[7].ANGLE   = (can2_RxData[0] << 8) | can2_RxData[1];  // 转子机械角度
			MOTOR_REAL_INFO[7].RPM     = (can2_RxData[2] << 8) | can2_RxData[3];  // 实际转子转速
			MOTOR_REAL_INFO[7].CURRENT = (can2_RxData[4] << 8) | can2_RxData[5];  // 实际转矩电流
		}; break;
		
		case M3508_CHASSIS_MOTOR_ID_3:
		{ 
			MOTOR_REAL_INFO[8].ANGLE   = (can2_RxData[0] << 8) | can2_RxData[1];  // 转子机械角度
			MOTOR_REAL_INFO[8].RPM     = (can2_RxData[2] << 8) | can2_RxData[3];  // 实际转子转速
			MOTOR_REAL_INFO[8].CURRENT = (can2_RxData[4] << 8) | can2_RxData[5];  // 实际转矩电流
		}; break;	
		case M3508_CHASSIS_MOTOR_ID_4:
		{ 
			MOTOR_REAL_INFO[9].ANGLE   = (can2_RxData[0] << 8) | can2_RxData[1];  // 转子机械角度
			MOTOR_REAL_INFO[9].RPM     = (can2_RxData[2] << 8) | can2_RxData[3];  // 实际转子转速
			MOTOR_REAL_INFO[9].CURRENT = (can2_RxData[4] << 8) | can2_RxData[5];  // 实际转矩电流
		}; break;
		

		case M3508_CHASSIS_MOTOR_ID_5:
		{ 
			MOTOR_REAL_INFO[10].ANGLE   = (can2_RxData[0] << 8) | can2_RxData[1];  // 转子机械角度
			MOTOR_REAL_INFO[10].RPM     = (can2_RxData[2] << 8) | can2_RxData[3];  // 实际转子转速
			MOTOR_REAL_INFO[10].CURRENT = (can2_RxData[4] << 8) | can2_RxData[5];  // 实际转矩电流
		}; break;
		case M3508_CHASSIS_MOTOR_ID_6:
		{ 
			MOTOR_REAL_INFO[11].ANGLE   = (can2_RxData[0] << 8) | can2_RxData[1];  // 转子机械角度
			MOTOR_REAL_INFO[11].RPM     = (can2_RxData[2] << 8) | can2_RxData[3];  // 实际转子转速
			MOTOR_REAL_INFO[11].CURRENT = (can2_RxData[4] << 8) | can2_RxData[5];  // 实际转矩电流
		}; break;
...
{
//	MOTOR_REAL_INFO->unitMode=CURRENT_MODE;
//	MOTOR_REAL_INFO->TARGET_CURRENT=0.0f;
	CurrentCrl(MOTOR_REAL_INFO,0);
	PID_Reset(pp);
